...
-   React SPA (lightweight solution) with Node.js backend
...
-   Responsive web app only (no native mobile apps)
...
-   Business may be deactivated (temporary, user-initiated by Owner) or suspended (admin-initiated). Both prevent the business from being listed and disable the business website; only admin can lift a suspension.
...
-   **Content storage**: all content stored in a single JSON file per business (stored on VPS disk; database tables maintain searchable list)
-   **Content management**: users manage all fields through config panel
-   **Multi-language**: business content supports multiple languages; users enter translations via config panel
-   **Website generation**: JSON file used to generate the single-page website
...
-   **Versioning**: not included in MVP (future enhancement)
